...
def index():
    ...
    return render_template("index.html", workers=workers_on_view, objects=objects_on_view)

def worker_to_view(worker: Worker, keepalive: int, objects: list) -> dict:
    last_keep_alive = ''
    if (keepalive):
        last_keep_alive = datetime.fromtimestamp(float(keepalive)).strftime('%d/%m/%Y %H:%M:%S')
    return {
         "host": worker.host,
